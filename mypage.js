...
async function load(){
  ...
  const profile=await window.SSALMUCK.getProfile(user.id);document.getElementById('nickname').value=profile?.nickname||'';document.getElementById('region').value=profile?.region||'';document.getElementById('displayNickname').textContent=profile?.nickname||'닉네임을 설정해주세요';
...
document.getElementById('profileForm').addEventListener('submit',async e=>{e.preventDefault();const nickname=document.getElementById('nickname').value.trim(),region=document.getElementById('region').value||null;if(nickname&&nickname.length<2)return showMessage(message,'닉네임은 2자 이상 입력해주세요.','error');const {error}=await client.from('profiles').update({nickname:nickname||null,region}).eq('id',user.id);if(error)return showMessage(message,error.message,'error');document.getElementById('displayNickname').textContent=nickname||'닉네임을 설정해주세요';showMessage(message,'정보를 저장했습니다.')});
